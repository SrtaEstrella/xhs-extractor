...
- **LLM 文档整理**：OCR 完成后可调用 DeepSeek 将碎片文本整理为连贯文章，支持流式输出和左右双栏对照
...
启动后浏览器会同时打开小红书和工具界面两个标签页。在输入框中粘贴小红书分享文本或笔记 URL 即可提取。正文和图片会即刻展示，提取完成后可随时点击 OCR 识别、下载图片、下载正文，无需重建任务。
...
│   ├── ocr.py               # OCR 识别
│   ├── llm_config.py        # LLM API 配置（加密存储）
│   ├── llm_process.py       # LLM 文档整理
│   ├── models.py            # 数据模型
│   ├── cli.py               # 命令行工具
│   ├── web_app.py           # Web 应用
│   └── ...
